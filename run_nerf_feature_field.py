...
dataset = Dataset()
feature_field = FeatureField(features_per_level = 32).cuda()
optimizer = torch.optim.Adam([w1, w2, w3, b1, b2, b3, feature_field.hashtable], lr=5e-3) 
...
x, z_vals, target_image, viewdirs = dataset.get_data(0)
# x_encoded = feature_field.encode(x)
# print(x.reshape(-1, 3).reshape(x.shape).shape)
# print(target_image.shape)

# print(feature_field.encode(torch.tensor([1, 2, 3], device='cuda:0')))

for i in range(iterations):
    img_i = 0  #img_i = np.random.randint(100)
    x, z_vals, target_image, viewdirs = dataset.get_data(img_i)
    encoded_x = feature_field.encode(x)
